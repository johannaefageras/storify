...
Du har rätt att veta vem som läser dina texter, hur systemet fungerar under ytan och var dina data fysiskt bor. Här är exakt vad som händer med det du skriver i verktyget.
...
När du skriver i [Wizarden](https://www.mystorify.se/wizard) eller fyller i [AI-intervjun](https://www.mystorify.se/interview) skickas ditt material till Anthropics Claude-API för att generera det färdiga inlägget. Det är så tjänsten fungerar. Anthropic använder inte API-data för att träna sina modeller. Deras affärsmodell för API-kunder bygger på att datat förblir privat. Materialet bearbetas enbart för att utföra den uppgift du bad om, ett formaterat inlägg returneras till oss, och datat ligger sedan inte kvar hos dem för annat bruk. Sen sparas inlägget i din egen Storify-databas, direkt och exklusivt kopplat till ditt konto.

AI:n läser ditt material i ögonblicket den genererar ett inlägg. Den minns det inte mellan sessioner. Den lär sig inte av dig över tid. Varje generering är helt fristående. Verktyget bygger ingen dold profil av din personlighet i bakgrunden.
...
För användare som inte vill skapa ett konto sparas Wizardens profil i webbläsarens localStorage istället för i en central databas. Webbläsarens localStorage är en liten isolerad textfil på din egen maskin.
